...
if cargo run --quiet --bin avon -- examples/test.av --deploy alice --root "$root"; then
...
if cargo run --quiet --bin avon -- examples/list_insert.av --deploy test --root "$root"; then
...
out=$(cargo run --quiet --bin avon -- eval examples/import_example.av)
if [[ "$out" == "[one, two]" ]]; then
  echo "OK"
else
...
out=$(cargo run --quiet --bin avon -- eval examples/map_example.av)
if [[ "$out" == "[a-, b-, c-]" ]]; then
  echo "OK"
else
...
out=$(cargo run --quiet --bin avon -- eval examples/fold_example.av)
...
if cargo run --quiet --bin avon -- examples/site_generator.av --deploy --root "$root"; then
  ok=1
...
if cargo run --quiet --bin avon -- examples/multi_file_deploy.av --deploy --root "$root"; then
...
if cargo run --quiet --bin avon -- examples/complex_template.av --deploy --root "$root"; then
...
if cargo run --quiet --bin avon -- examples/large_program.av --deploy --root "$root"; then
...
out=$(cargo run --quiet --bin avon -- eval examples/features_demo.av)
if [[ -n "$out" ]]; then
  echo "OK"
else
...
if cargo run --quiet --bin avon -- examples/complex_usage_1.av --deploy --root "$root"; then
...
if cargo run --quiet --bin avon -- examples/complex_usage_2.av --deploy --root "$root"; then
...
if cargo run --quiet --bin avon -- examples/complex_usage_3.av --deploy --root "$root"; then
...
if cargo run --quiet --bin avon -- examples/complex_usage_4.av --deploy --root "$root"; then
...
if cargo run --quiet --bin avon -- examples/complex_usage_5.av --deploy --root "$root"; then
...
out=$(cargo run --quiet --bin avon -- eval examples/plus_strings.av)
if [[ -n "$out" ]]; then
  echo "OK"
else
...
if cargo run --quiet --bin avon -- examples/plus_lists.av --deploy --root "$root"; then
...
if cargo run --quiet --bin avon -- examples/operators_demo.av --deploy --root "$root"; then
...
cargo run --quiet --bin avon -- examples/test.av --deploy alice --root "$root" 2> /tmp/stderr.txt || true
...
# test --force
cargo run --quiet --bin avon -- examples/test.av --deploy alice --root "$root" --force
if [ $? -ne 0 ]; then
...
for file in "${remaining_files[@]}"; do
  if cargo run --quiet --bin avon -- eval "examples/$file" > /dev/null 2>&1; then
...
out=$(cargo run --quiet --bin avon -- eval examples/filter_example.av)
if [[ "$out" == "[a, b, c]" ]]; then
  echo "OK"
else
...
out=$(cargo run --quiet --bin avon -- eval examples/string_functions.av)
if [[ -n "$out" ]]; then
  echo "OK"
else
...
out=$(cargo run --quiet --bin avon -- eval examples/split_join.av)
if [[ -n "$out" ]]; then
  echo "OK"
else
...
out=$(cargo run --quiet --bin avon -- eval examples/join_replace.av)
if [[ -n "$out" ]]; then
  echo "OK"
else
...
out=$(cargo run --quiet --bin avon -- eval examples/contains_starts_ends.av)
if [[ -n "$out" ]]; then
  echo "OK"
else
...
out=$(cargo run --quiet --bin avon -- eval examples/map_filter_fold.av)
if [[ -n "$out" ]]; then
  echo "OK"
else
...
out=$(cargo run --quiet --bin avon -- eval examples/nested_let.av)
if [[ -n "$out" ]]; then
  echo "OK"
else
...
out=$(cargo run --quiet --bin avon -- eval examples/let_cascade.av)
if [[ -n "$out" ]]; then
  echo "OK"
else
...
if cargo run --quiet --bin avon -- examples/package_json_gen.av --deploy --root "$root"; then
...
if cargo run --quiet --bin avon -- examples/docker_compose_gen.av --deploy --root "$root"; then
...
if cargo run --quiet --bin avon -- examples/vim_simple.av --deploy --root "$root"; then
...
if cargo run --quiet --bin avon -- examples/neovim_simple.av --deploy --root "$root"; then
...
if cargo run --quiet --bin avon -- examples/emacs_init.av --deploy --root "$root"; then
...
if cargo run --quiet --bin avon -- examples/github_actions_gen.av --deploy --root "$root"; then
...
for i in 1 2 3 4 5; do
  if cargo run --quiet --bin avon -- eval "examples/curly_test_${i}_"*.av > /dev/null 2>&1; then
    curly_ok=$((curly_ok + 1))
  else
